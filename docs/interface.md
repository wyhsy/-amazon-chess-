...
>
> 状态：**已冻结 v1.0（2026-09-13，组长定稿）**——第 1 节的 5 个决策点已全部采纳并冻结，作为全队编码的唯一契约。
> 如需调整接口，走第 8 节的变更流程（由组长评估后统一同步给队友）。
...
| # | 决策点 | 结论（v1.0 冻结） | 理由 |
| --- | --- | --- | --- |
| D1 | 棋盘表示 | **一维 `list[int]`，长度 100，索引 `r * 10 + c`** | 拷贝/哈希极快（AI 搜索每步都要复制棋盘），比 10×10 嵌套 list 快得多 |
| D2 | 坐标顺序 | **`(row, col)`，row=0 为最上面一行，col=0 为最左列** | 与打印输出、二维思维一致；渲染时 `x=col, y=row` |
| D3 | 走法结构 | **`Move = namedtuple("Move", "src dst arrow")`**，三个元素都是 `(row, col)` | 可读、可解包、比自定义类轻；`from` 是关键字，不能作字段名 |
| D4 | 走法生成粒度 | **两种都提供**：`legal_moves()` 一次性生成全部完整走法（UI/测试用）、`piece_targets()` + `shot_targets()` 分步生成（AI 搜索用，可按需剪枝） | UI 要方便，AI 要性能和剪枝空间 |
| D5 | 棋盘可变性 | **`apply_move()` 返回新棋盘（不可变语义）**；AI 内部若性能不足，另用 `do_move()` / `undo_move()` 就地回溯 | 不可变语义最不容易写出 bug；回溯版是纯性能优化，接口分开以免污染语义 |

**以上为组长定稿结论，直接执行，不要在 PR 里重新争论。** 真有必要调整，走第 8 节流程。
...
> ⚠️ **标准开局以课程题目册/老师给的图示为准**（行 0 在顶还是底、黑白谁在哪个角）。
> 组长核对后把结论写成 `test/test_rule.py::test_initial_board` 的断言；之后一律以测试为准，禁止口头约定漂移。
> 若图示与上表不一致，以图示为准并更新本文件。
...
1. 队友如果认为某个接口需要改，先在群里说明理由，**不要直接改别人的模块签名**。
2. 组长确认后，在 GitHub 开一个 Issue（标题以 `[interface]` 开头），写清改什么、为什么、影响谁。
3. 修改代码 + **同一 PR 内更新本文件**（版本号 +1，并在第 9 节登记）。
4. 排查并同步所有调用点：`grep -rn "被改的函数名" .`
5. 合并后，组长把变更点直接发给受影响模块的同学，并确认对方已同步。

## 9. 变更记录

| 版本 | 日期 | 改动 | 作者 |
| --- | --- | --- | --- |
| v0.1 | 2026-09-13 | 初稿：确定模块边界与签名草案 | @wyhsy |
| **v1.0** | **2026-09-13** | **冻结 D1~D5：棋盘一维数组、`(row, col)` 坐标、`Move` namedtuple、双粒度走法生成、不可变 + 回溯两套 API；作为全队编码契约** | **@wyhsy** |
